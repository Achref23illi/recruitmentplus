...
const AiAssistantPage = () => {
  ...
  const generateAIResponse = async (userQuery: string): Promise<string> => {
    ...
    try {
      // Check if the previous message was asking for a job position
      if (messages.length >= 2) {
        const previousMessage = messages[messages.length - 2];
        if (previousMessage.sender === 'assistant' && 
            (previousMessage.content.includes("What position would you like me to create a job description for?") ||
             previousMessage.content.includes("What position would you like me to create interview questions for?"))) {
          
          // This is a response to our position query
          const position = userQuery.trim();
          
          if (previousMessage.content.includes("job description")) {
            return await openai.job.generateJobDescriptionService(position, 'Your Company');
          } else if (previousMessage.content.includes("interview questions")) {
      ...
      if (lowerQuery.includes('job description') || lowerQuery.includes('draft a job description')) {
        // Check if user provided a position in the query
        const positionMatch = lowerQuery.match(/job description for (?:an? )?(.*?)(?:\s+position)?$/i) || 
                             lowerQuery.match(/draft a job description for (?:an? )?(.*?)(?:\s+position)?$/i);
        
        if (positionMatch && positionMatch[1]?.trim() && positionMatch[1].trim() !== 'a' && positionMatch[1].trim() !== 'an') {
          // User provided a position, generate directly
          const position = positionMatch[1].trim();
          return await openai.job.generateJobDescriptionService(position, 'Your Company');
        } else {
          // User didn't provide a position, ask for it
          return "I'd be happy to help you create a job description! What position would you like me to create a job description for? \n\nFor example: Software Engineer, Marketing Manager, Sales Representative, etc.";
        }
      }
      
      if (lowerQuery.includes('interview questions') || lowerQuery.includes('create interview questions')) {
        // Check if user provided a position in the query
        const positionMatch = lowerQuery.match(/interview questions for (?:an? )?(.*?)(?:\s+position)?$/i) || 
                             lowerQuery.match(/create interview questions for (?:an? )?(.*?)(?:\s+position)?$/i);
        ...
        if (positionMatch && positionMatch[1]?.trim() && positionMatch[1].trim() !== 'a' && positionMatch[1].trim() !== 'an') {
          ...
          // User didn't provide a position, ask for it
          return "I'd be happy to help you create interview questions! What position would you like me to create interview questions for? \n\nFor example: Software Developer, Product Manager, Data Analyst, etc.";
        }
      }
  ...
  const handleGenerateGenericJobDescription = async () => {
    ...
    // Instead of generating directly, ask for the position
    addMessage({ 
      content: "I'd be happy to help you create a job description! What position would you like me to create a job description for? \n\nFor example: Software Engineer, Marketing Manager, Sales Representative, etc.", 
      sender: 'assistant'
    });
  };
  ...
  const handleGenerateGenericInterviewQuestions = async () => {
    ...
    // Instead of generating directly, ask for the position
    addMessage({ 
      content: "I'd be happy to help you create interview questions! What position would you like me to create interview questions for? \n\nFor example: Software Developer, Product Manager, Data Analyst, etc.", 
      sender: 'assistant'
    });
  };
